...
HRESULT image::init(const char * fileName, int width, int height, BOOL trans, COLORREF transColor, BOOL isblend)
{
	if (_imageInfo != NULL) release();
	if (_blendImage != NULL) release();
...
HRESULT image::init(const char* fileName, float x, float y, int width, int height, int frameX, int frameY, BOOL trans, COLORREF transColor, BOOL isblend)
{
	if (_imageInfo != NULL) release();
	if (_blendImage != NULL) release();
...
HRESULT image::init(const char* fileName, int width, int height, int frameX, int frameY, BOOL trans, COLORREF transColor, BOOL isblend)
{
	if (_imageInfo != NULL) release();
	if (_blendImage != NULL) release();
...
void image::alphaFrameRender(HDC hdc, int destX, int destY, BYTE alpha)
{
	...
	//{
	//	//이미지를 먼저 복사합시다
	//	BitBlt(_blendImage->hMemDC, destX, destY, _imageInfo->frameWidth, _imageInfo->frameHeight,
	//		hdc, _imageInfo->frameWidth, _imageInfo->frameHeight, SRCCOPY);

	//	//복사해온 이미지에서 특정칼라(보통 마젠타)를 제거
	//	GdiTransparentBlt(_blendImage->hMemDC, destX, destY, _imageInfo->frameWidth,
	//		_imageInfo->frameHeight, _imageInfo->hMemDC, destX, destY, _imageInfo->frameWidth,
	//		_imageInfo->frameHeight, _transColor);
	//	
	//	//알파 적용
	//	AlphaBlend(hdc, destX, destY, _imageInfo->frameWidth, _imageInfo->frameHeight,
	//		_blendImage->hMemDC, destX, destY, _imageInfo->frameWidth,
	//		_imageInfo->frameHeight, _blendFunc);
	//}
	//else
	//{
	//	AlphaBlend(hdc, destX, destY, _imageInfo->frameWidth, _imageInfo->frameHeight,
	//		_imageInfo->hMemDC, destX, destY, _imageInfo->frameWidth,
	//		_imageInfo->frameHeight, _blendFunc);
	//}
}
...
void image::alphaFrameRender(HDC hdc, int destX, int destY, int currentFrameX, int currentFrameY, BYTE alpha)
{
	...
	if (_trans)
	{
		...
		//이미지를 먼저 복사합시다
		BitBlt(_blendImage->hMemDC, _imageInfo->currentFrameX * _imageInfo->frameWidth, _imageInfo->currentFrameY * _imageInfo->frameHeight, _imageInfo->frameWidth, _imageInfo->frameHeight,
			hdc, destX, destY, SRCCOPY);
